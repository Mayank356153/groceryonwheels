...
export default function NewItem() {
  ...
  try {
    ...
    setLoading(false);
     axios
      .get(`api/items/available-images`, {
         headers: {
  ...
  console.log(pair[0] + ": " + pair[1]);
    ...
    const url = id ? `api/items/${id}` : `api/items/create/sql`;
  ...
  const onSearch=async(query)=>{
    try {
        if(query === "") return;
      const { data } = await axios.get(`api/new-items/search?q=${query}`,{
         headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`
      },
      });
...
function findImagesByCode(imageCode, preloadedImages) {
  if (!imageCode) return [];

  const matched = Object.entries(preloadedImages)
    .filter(([orig]) =>
  ...
  const setResult=(item)=>{
    ...
    setFormData({
        ...
        itemImages:findImagesByCode(item.ImageCode, preloadedImages.current) || [],
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      >
        ...
        {value && (
          <button
            type="button"
            onClick={() => setValue("")}
            className="mx-2 rounded-full p-1 hover:bg-gray-100"
            aria-label="Clear"
            title="Clear"
          >
            <svg
        ...
        <button
          ...
          className="m-1 px-3 py-2 text-sm font-medium rounded-lg bg-blue-600 text-white hover:bg-blue-700"
        >
          Search
        </button>
      </div>
      ...
      {value && searchResults.length > 0 && (
        <div className="mt-3 bg-white rounded-lg shadow border border-gray-200 absolute z-10  max-h-40 overflow-y-auto">
          {searchResults.map((item) => (
            ...
            <div className="mt-4 flex flex-col gap-5 p-4 bg-white border-t-2 rounded-lg border-t-blue-600">
                ...
                {formData.itemImages.length > 0 && (
                  <div className="mt-2">
                    ...
                    <ul className="list-disc pl-5">
                      {formData.itemImages.map((file, index) => (
                        <li key={index} className="text-sm flex items-center">
                          {formData.itemImages.length} {file}
                          <button
                            type="button"
                            onClick={() =>
                              setFormData((prev) => ({
