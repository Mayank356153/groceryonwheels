...
const OneCategoryMaster = ({ onClose, category }) => {
  ...
  const handleRegenerateMasterImage = async () => {
    try {
      ...
      `/api/items/category/getImageForCategory?categoryId=${category._id}`,
      {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="relative w-11/12 max-w-4xl p-6 bg-white rounded-lg shadow-lg">
        ...
        <h2 className="mb-4 text-xl font-semibold text-center">Master Images - {category?.name || "NA"}</h2>
        <p className="mb-4 text-center text-gray-500">Category ID: {category?._id || "NA"}</p>
